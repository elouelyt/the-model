...
| **Sprint 4** | Deploy to GCP + mateogrisales.com | 1. Expose prediction pipeline as GCP Cloud Run API. 2. Build minimal React UI in Lovable. 3. Connect frontend to backend API. 4. Deploy live at mateogrisales.com. | Live public demo at `orange-court-ai.lovable.app` with the ranking-based model | 8–10 hrs | TBD | ✅ Done |
| **Sprint 4b** | GitHub Pages Static Site | 1. `generate_html.py` runs full pipeline and generates dark-themed `index.html`. 2. GitHub Actions daily cron at 08:00 UTC commits updated HTML. 3. Minimal `requirements-generate.txt` — no GCP deps. 4. `cloudscraper` replaces `requests` to bypass ATP website WAF. | Zero-cost daily-updating public page at `elouelyt.github.io/the-model` | 2 hrs | Apr 2026 | ✅ Done |
| **Sprint 5** | LangGraph Agent Architecture | ✓ 1. Introduce LangGraph as coordinator layer. ✓ 2. Refactor sub-agents (odds, rankings, model, explanation) into LangGraph nodes. ✓ 3. Add retry/fallback logic (rankings retry ×2, sentiment defaults to neutral). ✓ 4. Conditional routing based on data availability. | `src/pipeline/coordinator.py` — StateGraph with 4 nodes; `generate_html.py` delegates to coordinator | 10–12 hrs | Apr 2026 | ✅ Done |
| **Sprint 6** | Data Enrichment & Model Upgrade | ✅ 1. Surface/conditions feature. ✅ 2. H2H data (Sackmann, 4 yrs, per-surface). ✅ 3. News/sentiment agent (RSS + keyword scoring, parallel). ✅ 4. Logistic regression model (63.7% CV, serialized to JSON). ✅ 5. `docs/ML_ENGINEER.md` persona. | Surface badges, H2H bars, sentiment flags, LR model live on GitHub Pages | 12–15 hrs | Apr 2026 | ✅ Done |
...
- [x] End-to-end smoke test — frontend confirmed working at `orange-court-ai.lovable.app`
